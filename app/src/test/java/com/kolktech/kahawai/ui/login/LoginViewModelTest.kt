...
class LoginViewModelTest {
    ...
    @Test
    fun `setup with any blank field sets error state without calling the network`() = runTest {
        // The blank-field check runs synchronously before any suspension, and
        // every case produces the same Error(message) value, which StateFlow
        // would conflate into a single Turbine emission — assert .value
        // directly after each call instead of awaiting distinct emissions.
        val expected = "res:${R.string.login_enter_setup_fields}"

        vm().apply {
            setup("", "user", "pass")
            assertEquals(expected, (state.value as LoginState.Error).message)
        }
        vm().apply {
            setup("token", "", "pass")
            assertEquals(expected, (state.value as LoginState.Error).message)
    ...
    @Test
    fun `setup success saves tokens and transitions to LoggedIn`() = runTest {
        server.enqueue(MockResponse().setBody("""{"access_token":"tok2","refresh_token":"ref2","expires_in":3600}"""))
        val viewModel = vm()

        viewModel.state.test {
            assertEquals(LoginState.Idle, awaitItem())
            viewModel.setup("setup-token", "user", "pass")
            var item = awaitItem()
            if (item is LoginState.Loading) item = awaitItem()
            assertEquals(LoginState.LoggedIn, item)
        }
        coVerify(exactly = 1) { tokenStore.save(match { it.accessToken == "tok2" }) }
    }

    @Test
    fun `setup failure surfaces the formatted error sentinel`() = runTest {
        server.enqueue(MockResponse().setResponseCode(409).setBody("already set up"))
        val viewModel = vm()
        ...
        viewModel.state.test {
            ...
            if (item is LoginState.Loading) item = awaitItem()
            val error = item as LoginState.Error
            assertEquals("res:${R.string.login_setup_failed}:already set up", error.message)
        }
    }
}
